...
urlpatterns = [
    ...
    path('frontModify/<str:schoolRecordId>/', FrontModifyView.as_view(), name='frontModify'),  # 前台更新
    path('frontList/', FrontListView.as_view(), name='frontList'),  # 前台查询列表
    path('frontShow/<str:schoolRecordId>/', FrontShowView.as_view(), name='frontShow'),  # 前台显示详情页
    path('listAll/', ListAllView.as_view(), name='listAll'),  # 前台查询所有信息

    path('update/<str:schoolRecordId>/', UpdateView.as_view(), name='update'),  # Ajax方式更新记录

    path('add/', AddView.as_view(), name='add'),  # 后台添加
    path('backModify/<str:schoolRecordId>/', BackModifyView.as_view(), name="backModify"),  # 后台更新
